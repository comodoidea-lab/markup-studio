...
Cloudflare Pages にデプロイした場合、**Workers AI**（`@cf/google/gemma-3-12b-it`）が `/api/generate` 経由で使われます。APIキー不要です。

ローカル単体開発（`npm run dev`）では `/api/generate` が無いため、従来どおり BYOK にフォールバックします。Functions 込みの開発は `npm run dev:cf` を使います。
...
`wrangler.toml` と `functions/api/generate.ts` で Workers AI バックエンドを提供します。

```bash
npm install
npm run build
npm run pages:deploy          # Cloudflare Pages へデプロイ
npm run dev:cf                # Vite + Pages Functions をローカルで同時起動
```
...
技術スタック: Vite + React + TypeScript + Zustand + Tailwind CSS + Cloudflare Pages Functions。
